    ...
    <button
      ...
      className={`w-full text-left rounded-xl px-3 py-2.5 transition-all border ${
        active
          ...
          : 'border-transparent text-navy-200 hover:bg-white/[0.06] hover:text-white'
      }`}
    >
      <div className="flex items-start gap-2">
        {index != null && (
          <span
            className={`mt-0.5 w-6 h-6 rounded-lg text-xs font-bold flex items-center justify-center shrink-0 ${
              active ? 'bg-gold-500/20 text-gold-300' : 'bg-slate-800 text-slate-400'
            }`}
          >
            {index}
          </span>
        ...
        <div className="min-w-0 flex-1">
          ...
          {subtitle && <div className="text-xs font-mono text-navy-300 truncate">{subtitle}</div>}
          {(meta || dirty || badges.length > 0) && (
            <div className="mt-1 flex items-center gap-2 flex-wrap text-xs">
              {meta && <span className="text-navy-400">{meta}</span>}
              {badges.map((badge) => (
...
export default function AdminMasterDetail({
  ...
  return (
    <div className={`grid grid-cols-1 ${sidebarWidthClass} gap-4 ${minHeightClass}`}>
      <div className="sf-console-panel p-3 lg:max-h-[calc(100vh-280px)] lg:overflow-y-auto">
        {listTitle && (
          <div className="px-2 py-2 mb-1 border-b border-white/5">
            ...
            {listHint && <p className="text-xs text-navy-400 mt-1 leading-relaxed">{listHint}</p>}
          </div>
        )}
        {listHeader}
        <div className="space-y-1">
          {items.length === 0 ? (
            emptyList || <p className="px-2 py-4 text-sm text-navy-400">暂无数据</p>
          ) : onReorder ? (
            <OrchestrationDraggableList
              items={items}
              getId={getReorderId || getId}
              onReorder={onReorder}
              renderItem={(item, ctx) => {
                const id = getId(item)
      ...
      <div className="sf-console-panel p-5 lg:max-h-[calc(100vh-280px)] lg:overflow-y-auto">
        {(detailTitle || detailHint) && (
          <div className="mb-4 pb-3 border-b border-white/5">
            {detailTitle && <h3 className="text-sm font-semibold text-white">{detailTitle}</h3>}
            {detailHint && <p className="text-xs text-navy-400 mt-1 leading-relaxed">{detailHint}</p>}
          </div>
        )}
        {(selected || showDetail) ? (
          renderDetail(selected)
        ) : (
          <div className="h-full min-h-[240px] flex items-center justify-center text-navy-400 text-sm">
            {emptyDetail}
          </div>
        )}
      </div>
