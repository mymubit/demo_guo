...
import { Cpu, ExternalLink, Plus, Save, Trash2, Zap } from 'lucide-react'
    ...
    <div className="mx-auto max-w-6xl space-y-6 p-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-semibold text-ink">
            <Cpu className="h-6 w-6 text-navy-700" />
            模型管理
          </h1>
          <p className="mt-1 text-sm text-ink-muted">
            优先接入国内主流大模型（DeepSeek / 通义 / 智谱 / Kimi / 豆包）。密钥仅写入不回显。
          </p>
        </div>
        <Link to="/admin/config" className="text-sm text-navy-600 underline-offset-2 hover:underline">
          技能覆盖配置 →
        </Link>
      </div>
      ...
      {runtime ? (
        <div className="rounded-xl border border-slate-200 bg-white px-5 py-4">
          <div className="text-xs font-medium uppercase tracking-wide text-slate-400">运行时状态</div>
        ...
        <div className="mt-4 space-y-4">
          {vendorGroups.map((group) => (
            <div key={group.vendor}>
              <div className="mb-2 text-xs font-medium text-slate-500">{group.vendorLabel}</div>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      selectedPresetId === preset.id
                        ...
                        : 'border-slate-200 bg-slate-50 text-ink hover:border-navy-300 hover:bg-white',
                    )}
                  >
                    {preset.name}
                  </button>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
        <section className="space-y-3">
          ...
          {providers.length === 0 ? (
            <div className="rounded-xl border border-dashed border-slate-300 px-4 py-8 text-center text-sm text-ink-muted">
