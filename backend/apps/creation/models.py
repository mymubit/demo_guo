...
class SubSkillExecutionLog(models.Model):
    ...
    input_summary = models.JSONField("输入摘要", default=dict, blank=True)
    output_summary = models.JSONField("输出摘要", default=dict, blank=True)
    input_payload = models.JSONField("输入(完整)", default=dict, blank=True)
    output_payload = models.JSONField("输出(完整)", default=dict, blank=True)
    llm_io = models.JSONField("LLM 入出参", default=dict, blank=True)
    error_message = models.TextField("错误信息", blank=True, default="")
