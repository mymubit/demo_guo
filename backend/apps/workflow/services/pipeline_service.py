...
class WorkflowPipelineService:
    """主链编排 — FusionPipelineNode（活跃 Pack）为 SSOT。"""
    ...
    @classmethod
    def portal_hidden_fusion_node_ids(cls) -> frozenset:
        cls.ensure_defaults()
        hidden = {
            n["fusion_node_id"]
    ...
    @classmethod
    def creation_max_node_index(cls) -> int:
        chain = cls.portal_main_chain()
        return max((n["index"] for n in chain), default=5)

    @classmethod
    def creation_next_node_index(cls, current: int) -> Optional[int]:
        indices = sorted(n["index"] for n in cls.portal_main_chain())
        try:
    ...
    @classmethod
    def enabled_nodes(cls) -> List[Dict[str, Any]]:
        cls.ensure_defaults()
        return [
            {
                "index": n["index"],
    ...
    @classmethod
    def portal_main_chain(cls) -> List[Dict[str, Any]]:
        cls.ensure_defaults()
    ...
    @classmethod
    def node_meta_by_index(cls) -> Dict[int, Dict[str, Any]]:
        return {n["index"]: n for n in cls.portal_main_chain()}
    ...
    @classmethod
    def pipeline_nodes_for_creation(cls) -> List[Dict[str, Any]]:
        cls.ensure_defaults()
        out = []
        hidden = cls.portal_hidden_fusion_node_ids()
        for n in FusionNodeRegistry().main_chain_nodes():
