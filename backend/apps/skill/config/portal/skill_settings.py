...
class SkillConfigService:
    ...
    DEFAULT_CONFIGS: Dict[str, Dict[str, str]] = {
        ...
        'fusion.creation_form_overrides': {
            'value': '{}',
            'description': 'C 端创作表单运营覆盖（JSON，合并 SSOT catalog）',
        },
        'fusion.episode_settings_defaults': {
            'value': '{}',
            'description': '创作页集数默认值 JSON（覆盖代码兜底，空则使用内置默认）',
        },
        'fusion.default_user_prompt_template': {
            'value': (
                '请根据以下上游产物生成符合 Schema 的 JSON（只输出 JSON，无 markdown）：\n\n'
                '上游数据：\n{upstream_json}'
            ),
            'description': '融合节点 LLM 默认 user prompt 模板（{upstream_json} 占位符）',
        },
        # --- 质量审查 ---
        ...
        # --- 运行时队列（修改后需重启 dj_queue worker）---
        'runtime.dj_queue.queues': {
            'value': 'creation,default,evolve',
            'description': 'dj_queue 监听队列列表（逗号分隔；修改后需重启 worker）',
        },
        'runtime.dj_queue.mode': {
            'value': 'async',
            'description': 'dj_queue worker 模式（async/fork；修改后需重启 worker）',
        },
        'runtime.dj_queue.worker_threads': {
            'value': '2',
            'description': 'async 模式 worker 线程数（修改后需重启 worker）',
        },
    }
    ...
    @staticmethod
    def _normalize_llm_base_url(endpoint: str) -> str:
        """将完整 endpoint 转为 base_url（llm_service 会追加 /chat/completions）。"""
    ...
    @classmethod
    def migrate_legacy_llm_keys(cls) -> None:
        """旧 Admin 键名迁移到 skill 模块标准键名。"""
        legacy_map = {
            "llm.api_endpoint": "llm.base_url",
            "llm.model": "llm.model_name",
        }
        for old_key, new_key in legacy_map.items():
            old_obj = SkillConfig.objects.filter(config_key=old_key).first()
            if old_obj is None:
                continue
            if SkillConfig.objects.filter(config_key=new_key).exists():
                continue
            try:
                val = old_obj.get_decrypted_value() or ""
                if not val:
                    continue
                if new_key == "llm.base_url":
                    val = cls._normalize_llm_base_url(val)
                cls.set(new_key, val)
            except Exception as exc:  # noqa: BLE001
                logger.warning("迁移 LLM 配置 %s -> %s 失败: %s", old_key, new_key, exc)

    @classmethod
    def apply_env_llm_config(cls) -> None:
        """从环境变量灌入 LLM 配置（仅覆盖有值的项）。"""
        endpoint = getattr(settings, "LLM_API_ENDPOINT", "") or ""
        api_key = getattr(settings, "LLM_API_KEY", "") or ""
        if not endpoint and not api_key:
            return

        base_url = cls._normalize_llm_base_url(endpoint)
        if api_key:
            cls.set("llm.api_key", api_key)
        if base_url:
            cls.set("llm.base_url", base_url)
        if api_key and base_url:
            cls.set("llm.enabled", "true")
            logger.info("已从环境变量启用 LLM（llm.base_url=%s）", base_url)
...
class ThemeTemplateService:
    ...
    BUILTIN_THEMES: List[Dict[str, Any]] = [
        ...
        },
        {
            ...
            'sort_order': 75,
            'params': {
                'description': '真实生活痛点的共鸣与修复，给情绪正向出口（释怀/接受/和解），而非复仇逆袭',
                'act_ratio': [0.12, 0.22, 0.20, 0.18, 0.14, 0.14],
                'reversal_density': 0.15,
                'emotion_curve': [4, 3, 4, 5, 5, 6, 7, 8],
                'hook_types': ['痛点共鸣', '被理解时刻', '内心转变', '温暖出口'],
            },
            'hook_templates': [],
            'character_archetypes': [],
        },
...
def init_skill_data():
    ...
    SkillConfigService.migrate_legacy_llm_keys()
    SkillConfigService.apply_env_llm_config()
    from apps.skill.llm.providers import LlmProviderService

    LlmProviderService.migrate_legacy_skill_config()
    LlmProviderService.ensure_default_from_env()
    from apps.skill.llm.model_catalog import LlmCatalogService

    LlmCatalogService.ensure_seed_catalog()
    LlmProviderService.ensure_builtin_presets()
    ...
    CreationFormOverrideService.seed_defaults()
    ...
    ReferenceLibraryService.ensure_defaults()
    ThemeTemplateCatalogService.ensure_defaults()
    SkillRuleConfigService.ensure_defaults()
    AgentLlmRouteService.seed_defaults()
    ReviewScoringService.seed_defaults()
