...
FIXTURE_PATH = ROOT / "tools" / "fixtures" / "artifacts" / "valid-artifacts.json"
ERRORS: List[str] = []
...
def main() -> int:
    ...
    fixtures = {key: value for key, value in fixtures.items() if key in artifacts}
    fixtures["episode_plan"] = json.loads(
        (ROOT / "v6/evaluations/fixtures/episode-plan-valid.json").read_text(encoding="utf-8")
    )
