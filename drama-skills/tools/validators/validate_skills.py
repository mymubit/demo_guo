...
15. module_policy.evaluate_enable_when 全开；knowledge_policy.includes 路径可解析；
    includes 非空须显式 mode；mode=full 须 max_chars>0

用法：python tools/validators/validate_skills.py
"""
...
def check_agent_runtime(agent_ids: set) -> None:
    data = load_yaml(ROOT / "foundation/presets/agent-runtime.yaml")
...
def check_knowledge_orphans() -> None:
    ...
    index_files = {"knowledge-sections.md", "output-schemas.md", "README.md"}
    knowledge_files = [
...
def check_scoring_presets() -> None:
    ...
    presets = load_yaml(ROOT / "foundation" / "presets" / "scoring-presets.yaml")
...
def check_constraint_consistency() -> None:
    ...
    production = load_yaml(
        ROOT / "foundation" / "presets" / "production-feasibility.yaml"
    )
...
def check_role_policy_sections(role_metas: Dict[str, Dict[str, Any]]) -> None:
    """声明了 modules 的角色必须在 role.yaml 声明 rule_policy.sections（收窄 global 注入）。"""
    for agent_id, meta in role_metas.items():
        modules = meta.get("modules") or []
        if not modules:
            continue
        sections = ((meta.get("rule_policy") or {}).get("sections")) or []
        if not sections:
            err(f"{agent_id}: 声明了 modules 但缺少 rule_policy.sections")


def check_role_sections_index_sync(role_metas: Dict[str, Dict[str, Any]]) -> None:
    """role.yaml.rule_policy.sections 须与 knowledge-sections.md 角色表双向一致（role 为 SSOT）。"""
    mapping_path = ROOT / "knowledge" / "knowledge-sections.md"
    index_by_agent: Dict[str, set[str]] = {}
    for line in mapping_path.read_text(encoding="utf-8").splitlines():
        if not line.startswith("| drama."):
            continue
        cells = [c.strip() for c in line.strip("|").split("|")]
        if len(cells) < 2:
            continue
        agent_id = cells[0]
        index_by_agent[agent_id] = {
            s.strip() for s in cells[1].split(",") if s.strip()
        }
    for agent_id, meta in role_metas.items():
        role_sections = {
            str(s).strip()
            for s in ((meta.get("rule_policy") or {}).get("sections") or [])
            if str(s).strip()
        }
        if not role_sections:
            continue
        indexed = index_by_agent.get(agent_id)
        if indexed is None:
            err(f"knowledge-sections.md 缺少角色行: {agent_id}")
            continue
        if role_sections != indexed:
            only_role = sorted(role_sections - indexed)
            only_index = sorted(indexed - role_sections)
            err(
                f"{agent_id}: rule_policy.sections 与 knowledge-sections.md 不一致"
                f"（仅 role: {only_role or '-'}；仅索引: {only_index or '-'}）"
            )


_HOLLOW_ANTI_MARKERS = ("空壳", "占位", "过空", "口号")


def check_content_quality_gates(role_metas: Dict[str, Dict[str, Any]]) -> None:
    """阶段 D：fewshot≥2；存在 anti 时须含空壳/占位类反例。"""
    for agent_id, meta in role_metas.items():
        skill_rel = str(meta.get("skill_dir") or "").strip()
        if not skill_rel:
            continue
        skill_dir = ROOT / skill_rel.replace("\\", "/").lstrip("/")
        fewshot_path = skill_dir / "fewshots.v1.yaml"
        if fewshot_path.is_file():
            data = load_yaml(fewshot_path)
            shots = data.get("fewshots") or []
            if not isinstance(shots, list) or len(shots) < 2:
                err(f"{agent_id}: fewshots.v1.yaml 须至少 2 条具体正例（阶段 D 内容门禁）")
        anti_path = skill_dir / "anti-examples.yaml"
        if not anti_path.is_file():
            continue
        anti = load_yaml(anti_path)
        examples = anti.get("examples") or []
        if not isinstance(examples, list) or not examples:
            err(f"{agent_id}: anti-examples.yaml 缺少 examples")
            continue
        blob = json.dumps(examples, ensure_ascii=False)
        if not any(marker in blob for marker in _HOLLOW_ANTI_MARKERS):
            err(
                f"{agent_id}: anti-examples 须含至少一条空壳/占位/口号类反例"
                f"（标题或 note 含 {_HOLLOW_ANTI_MARKERS}）"
            )


def check_injection_policies(role_metas: Dict[str, Dict[str, Any]]) -> None:
    """注入策略契约：evaluate_enable_when；knowledge mode/includes；禁止盲灌 full。"""
    allowed_modes = {"index", "full", "off", "none", "disabled"}
    for agent_id, meta in role_metas.items():
        module_policy = meta.get("module_policy") or {}
        if not module_policy.get("evaluate_enable_when"):
            err(f"{agent_id}: module_policy.evaluate_enable_when 必须为 true")
        knowledge_policy = meta.get("knowledge_policy") or {}
        includes = knowledge_policy.get("includes") or []
        if not isinstance(includes, list):
            err(f"{agent_id}: knowledge_policy.includes 必须是列表")
            continue
        for rel in includes:
            path = ROOT / str(rel).replace("\\", "/").lstrip("/")
            if not path.is_file():
                err(f"{agent_id}: knowledge_policy.includes 缺失文件 {rel}")
        # C5：声明了 includes 必须显式 mode；mode 合法；full 须带正数预算
        if includes and "mode" not in knowledge_policy:
            err(f"{agent_id}: knowledge_policy.includes 非空时必须显式声明 mode")
        if "mode" in knowledge_policy:
            mode = str(knowledge_policy.get("mode") or "").strip().lower()
            if mode not in allowed_modes:
                err(
                    f"{agent_id}: knowledge_policy.mode 非法 `{mode}`，"
                    f"允许 {sorted(allowed_modes)}"
                )
            if mode == "full":
                max_chars = int(knowledge_policy.get("max_chars") or 0)
                if max_chars <= 0:
                    err(
                        f"{agent_id}: knowledge_policy.mode=full 时 "
                        "max_chars 必须为正整数（防盲灌）"
                    )

def check_field_writers_target_roles(role_metas: Dict[str, Dict[str, Any]]) -> None:
    """field_writers 指向的模块，其 catalog.target_roles 须包含该产物的 producer。"""
    artifacts = load_artifacts_contract()
    catalog = (load_yaml(ROOT / "modules" / "catalog.yaml") or {}).get("modules") or {}
    producers = producer_map(artifacts)
    field_writers = artifacts.get("field_writers") or {}
    for artifact_key, writers in field_writers.items():
        if not isinstance(writers, dict):
            continue
        producer = producers.get(artifact_key)
        if not producer:
            continue
        for field, module_rel in writers.items():
            module_id = Path(str(module_rel)).stem
            meta = catalog.get(module_id) or {}
            targets = meta.get("target_roles") or []
            if producer not in targets:
                err(
                    f"field_writers.{artifact_key}.{field}: "
                    f"module `{module_id}` target_roles 未包含 producer `{producer}`"
                )
...
def check_tools_layout() -> None:
    ...
    required = (
        ...
        "tools/validators/validate_skills.py",
        "tools/lib/condition_eval.py",
        "tools/lib/contracts_loader.py",
        "tools/fixtures/config/project-settings.json",
        "tools/fixtures/artifacts/valid-artifacts.json",
        "eval/tools/eval_role_llm.py",
    )
    for rel in required:
...
def check_evolution_slim() -> None:
    """E/双减：inspirations 仅保留 inspirations.md（禁止旧指针文件回潮）。"""
    ssot = ROOT / "inspirations" / "inspirations.md"
    if not ssot.is_file():
        err("阶段 E：缺少 inspirations/inspirations.md")
        return
    body = ssot.read_text(encoding="utf-8")
    for heading in ("## 钩子创意", "## 反转创意", "## 金句创意", "## 结构创意", "## 待验证模式"):
        if heading not in body:
            err(f"inspirations.md 缺少 section：{heading}")
    for name in ("hooks.md", "reversals.md", "dialogues.md", "structures.md", "new-patterns.md"):
        pointer = ROOT / "inspirations" / name
        if pointer.is_file():
            err(f"双减硬切：禁止保留指针文件 inspirations/{name}，内容应只在 inspirations.md")
...
def main() -> int:
    ...
    check_role_policy_sections(role_metas)
    check_role_sections_index_sync(role_metas)
    check_content_quality_gates(role_metas)
    check_injection_policies(role_metas)
    check_field_writers_target_roles(role_metas)
    check_atomic_rules()
    check_scoring_presets()
    check_tools_layout()
    check_evolution_slim()
