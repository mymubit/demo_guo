...
from export_workbench_schema import export_definition  # noqa: E402
...
def check_workbench() -> None:
    ...
    platforms = load_yaml("foundation/presets/platform-profiles.yaml")
    presets = load_yaml("foundation/presets/scoring-presets.yaml")
...
def check_generated_schemas() -> None:
    ...
    for relative_path, expected in sorted(generated_artifacts(ROOT).items()):
        ...
        if actual != expected:
            ERRORS.append(f"生成物漂移: {relative_path}（请运行 python tools/generators/generate_parameter_schemas.py）")
